/**
 * Starter data for a fresh database. The server calls seedDatabase() on its
 * first boot, and it can also be run by hand:
 *
...
/**
 * Fills an empty database: the owner account, then the starter menu and stock.
 * Safe to re-run — existing staff, menu and orders are left alone.
 * Pass { demo: true } to add 30 days of sample trading on top.
 */
export function seedDatabase({ demo = false } = {}) {
  const DEMO = demo;
  seed = 20260922;
...
// Command line: npm run seed [-- --demo] [-- --reset]
if (process.argv[1]?.endsWith('seed.js')) {
  ...
  seedDatabase({ demo: args.has('--demo') });
